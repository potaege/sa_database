...
app.post("/insertRequest", async ({ body }: { body: Request }) => {
  try {
    ...
    ("model","sn","type","rated","description","warranty","workID")
    VALUES (${model},${sn},${type},${rated},${description},${warranty},${workID})`;
  } catch (error: any) {
    return {
...
app.get("/getByID:/id", async (id) => {
  return await db.$queryRaw`SELECT "id","model","sn","type","rated","description","warranty","workID","addedDate"
